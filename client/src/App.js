...
import shibaArtifact from './artifacts/contracts/Shib.sol/Shib.json';
...
function App() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const bankContract = await new ethers.Contract("0x5FC8d32690cc91D4c39d9d3abcBD16989F875707", bankArtifact.abi)
  ...
  const getTokenContract = async (symbol, bankContract, provider) => {
    ...
    const abi = symbol === 'Matic' ? maticArtifact.abi : (symbol === 'Shib' ? shibaArtifact.abi : usdtArtifact.abi)
